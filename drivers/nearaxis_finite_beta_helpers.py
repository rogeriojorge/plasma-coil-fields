...
def fft_relative_difference(solution, response):
    """Independent Fourier route for the closed-form pressure response (spectral Boozer map)."""
    g = solution.geometry
    x = float(solution.inputs.etabar) / np.asarray(g.curvature)
    zeta, _ = _fft_response(x, np.asarray(solution.sigma), np.asarray(g.d_l_d_phi),
                            float(solution.axis_length) / (2 * np.pi), response["iotaN"], response["Cp"],
                            int(solution.inputs.axis.nfp), response["phi"], response["varphi"])
    return float(np.max(abs(zeta - response["zeta"])) / max(np.max(abs(response["zeta"])), 1e-30))
